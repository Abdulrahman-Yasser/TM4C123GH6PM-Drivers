...
void IntCtrl_Init(void)
 {
    ...
    for (i = 0; i  < IntCtrl_Interrupt_count_cfg; i++){
        ...
        if(Initialized_Interrupt_Container.container_ptr[i].int_num >= 16){
            ...
            {
            case 0:
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN0, ((Initialized_Interrupt_Container.container_ptr[i].int_num - 16) ));
                break;
            case 1:
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN1, ((Initialized_Interrupt_Container.container_ptr[i].int_num - 16) % (32*1)));
                break;
            case 2:
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN2, ((Initialized_Interrupt_Container.container_ptr[i].int_num - 16) % (32*2)));
                break;
            case 3:
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN3, ((Initialized_Interrupt_Container.container_ptr[i].int_num - 16) % (32*3)));
                break;
            case 4:
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN4, ((Initialized_Interrupt_Container.container_ptr[i].int_num - 16) % (32*4)));
                break;
            default:
                break;
            }
            ...
            if(Initialized_Interrupt_Container.container_ptr[i].int_num == Memory_Management){
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_SYSHNDCTRL, 16);
                REG_ORING_VALUE_NO_CASTING(NVIC_SYSPRI1, (Initialized_Interrupt_Container.container_ptr[i].int_priority << 5));
            }else if(Initialized_Interrupt_Container.container_ptr[i].int_num == Bus_Fault){
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_SYSHNDCTRL, 17);
                REG_ORING_VALUE_NO_CASTING(NVIC_SYSPRI1, (Initialized_Interrupt_Container.container_ptr[i].int_priority << 13));
            }else if(Initialized_Interrupt_Container.container_ptr[i].int_num == Usage_Fault){
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_SYSHNDCTRL, 18);
                REG_ORING_VALUE_NO_CASTING(NVIC_SYSPRI1, (Initialized_Interrupt_Container.container_ptr[i].int_priority << 21));
            }
            ...
            if(Initialized_Interrupt_Container.container_ptr[i].int_num == SVCall){
                REG_ORING_VALUE_NO_CASTING(NVIC_SYSPRI2, (Initialized_Interrupt_Container.container_ptr[i].int_priority << 29));
            }else if(Initialized_Interrupt_Container.container_ptr[i].int_num == Debug_Monitor){
                REG_ORING_VALUE_NO_CASTING(NVIC_SYSPRI3, (Initialized_Interrupt_Container.container_ptr[i].int_priority << 5));
            }else if(Initialized_Interrupt_Container.container_ptr[i].int_num == PendSV){
                REG_ORING_VALUE_NO_CASTING(NVIC_SYSPRI3, (Initialized_Interrupt_Container.container_ptr[i].int_priority << 21));
            }else if(Initialized_Interrupt_Container.container_ptr[i].int_num == SysTick){
                REG_ORING_VALUE_NO_CASTING(NVIC_SYSPRI3, (Initialized_Interrupt_Container.container_ptr[i].int_priority << 29));
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_STCTRL, 1);
                REG_ORING_ONE_BIT_NO_CASTING(NVIC_STCTRL, 0);
            }
        }
    }
 }
void IntCtrl_DisableInterrupt(IntCtrl_InterruptType_t InterruptID){
    if(InterruptID >= 16){
        ...
        {
        case 0:
            REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_EN0, ( (InterruptID - 16) ));
            break;
        case 1:
            REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_EN1, ( (InterruptID - 16) % (32*1)));
            break;
        case 2:
            REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_EN2, ( (InterruptID - 16) % (32*2)));
            break;
        case 3:
            REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_EN3, ( (InterruptID - 16) % (32*3)));
            break;
        case 4:
            REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_EN4, ( (InterruptID - 16) % (32*4)));
            break;
        default:
            break;
        }
        ...
        if(InterruptID == Memory_Management){
            REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_SYSHNDCTRL, 16);
        }else if(InterruptID == Bus_Fault){
            REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_SYSHNDCTRL, 17);
        }else if(InterruptID == Usage_Fault){
            REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_SYSHNDCTRL, 18);
        }
    }else if(InterruptID == SysTick){
        REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_STCTRL, 1);
        REG_CLEAR_ONE_BIT_NO_CASTING(NVIC_STCTRL, 0);
    }
}
...
void IntCtrl_Enable_Interrupt(IntCtrl_InterruptType_t InterruptID){
    if(InterruptID >= 16){
        ...
        {
        case 0:
            REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN0, ( (InterruptID - 16) ));
            break;
        case 1:
            REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN1, ( (InterruptID - 16) % (32*1)));
            break;
        case 2:
            REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN2, ( (InterruptID - 16) % (32*2)));
            break;
        case 3:
            REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN3, ( (InterruptID - 16) % (32*3)));
            break;
        case 4:
            REG_ORING_ONE_BIT_NO_CASTING(NVIC_EN4, ( (InterruptID - 16) % (32*4)));
            break;
        default:
            break;
        }
